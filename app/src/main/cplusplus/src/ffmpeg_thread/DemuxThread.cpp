...
void DemuxThread::run() {
    ...
    if (DEMUXTHREAD_DEBUG_ON)
        LOGD("DemuxThread Current thread id->%d", *(unsigned int *) &threadId);
    ...
    while (!mAbort) {
        ...
        if (mVideoIndex == pkt.stream_index) {
            ...
            if (DEMUXTHREAD_DEBUG_ON)
                LOGD("DemuxThread run put into video pkt queue");
        } else if (mAudioIndex == pkt.stream_index) {
            // put into audio pkt queue
            if (DEMUXTHREAD_DEBUG_ON)
                LOGD("DemuxThread run put into audio pkt queue");
        } else {
            //其他
            if (DEMUXTHREAD_DEBUG_ON)
                LOGD("DemuxThread run other stream index");
            av_packet_unref(&pkt); // 释放
        }
    }
    if (DEMUXTHREAD_DEBUG_ON)
        LOGD("DemuxThread run end!");
}
...
int DemuxThread::init(const char *url) {
    ...
    if (DEMUXTHREAD_DEBUG_ON)
        LOGD("DemuxThread init mVideoIndex->%d , mAudioIndex->%d", mVideoIndex, mAudioIndex);
